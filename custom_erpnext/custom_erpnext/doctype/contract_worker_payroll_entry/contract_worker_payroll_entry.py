...
class ContractWorkerPayrollEntry(Document):
	...
	@frappe.whitelist()
	def create_contract_worker_salary_slips(self):
		"""
			...
			args = frappe._dict(
				{
					...
					"contract_worker_payroll_entry": self.name,
...
def get_emp_list(sal_struct, cond, end_date, payroll_payable_account):# added condition to filter contct base employee and not mandatory to have salary_assignment
...
def create_worker_salary_slips_for_employees(employees, args, publish_progress=True):
	salary_slips_exists_for = get_existing_salary_slips(employees, args)
	count = 0
	salary_slips_not_created = []
	for emp in employees:
		if emp not in salary_slips_exists_for:
			args.update({"doctype": "Contract Worker Salary Slip", "employee": emp})
	...
	payroll_entry = frappe.get_doc("Contract Worker Payroll Entry", args.payroll_entry)
