...
erpnext.attendance_adjustment_tool = {
	load_employees: function(frm) {
		if(frm.doc.date) {
			frappe.call({
				method: "custom_erpnext.custom_erpnext.doctype.attendance_adjustment_tool.attendance_adjustment_tool.get_employees",
				args: {
					date: frm.doc.date,
					shift: frm.doc.select_shift,
					status: frm.doc.attendance_status,
					leave_types:frm.doc.leave_types,
	...
	make(frm, employee) {
		...
		mark_employee_toolbar.find(".btn-assign")
			...
			.on("click", function() {
				...
				frappe.call({
					method: "custom_erpnext.custom_erpnext.doctype.attendance_adjustment_tool.attendance_adjustment_tool.mark_employee_attendance",
					args:{
						"employee_list":employees_to_shift,
						"status": frm.doc.change_shift_to,
			...
			// 	});
			// 	frappe.call({
			// 		method: "custom_erpnext.custom_erpnext.doctype.attendance_adjustment_tool.attendance_adjustment_tool.mark_employee_attendance1",
			// 		args:{
			// 			"employee_list":employees_to_shift,
		...
		$.each(employee, function(i, m) {
			...
			<tr>\
				...
				<td style="width:10%; text-align:center;">\
						<label><class="employee-check""/>Date</label>\
				</td>\
				<td style="width:12%; text-align:center;">\
						<label><class="employee-check""/>Status</label>\
				</td>\
				<td style="width:17%; text-align:center;">\
						<label><class="employee-check""/>Leave Type</label>\
				</td>\
		...
		, )).appendTo(row); }
			...
			<tr>\
				...
				<td style="width:15%; text-align:center;">\
						<label><class="employee-check""/>%(employee5)s</label>\
				</td>\
				<td style="width:3%; text-align:center;">\
		...
		, {employee6:i+1,employee:m[0]+" : "+m[1],employee2:m[2],employee3:m[3],employee4:m[4],employee5:m[5] })).appendTo(row); //was: m.employee_name
		});
