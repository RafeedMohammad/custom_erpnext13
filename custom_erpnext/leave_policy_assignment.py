...
class override_LeavePolicyAssignment(Document):
	def validate(self):
		self.set_dates()
	...
	def get_leaves_for_passed_months(
		...
		if months_passed > 0:#added if condition as there is no option '18_days' in default option
			# so i added condition to overcome that keyerror given for 18_Days
			if leave_type_details.get(leave_type).earned_leave_frequency =="18_Days": #changed
				new_leaves_allocated= 0
			else:
